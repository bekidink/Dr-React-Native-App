const Logo = require("../assets/images/logo.png");
const Splashlogo = require("../assets/images/splash-logo.png");
const Onboarding1 = require("../assets/images/onboarding.png");
const Onboarding2 = require("../assets/images/onboarding2.png");
const Onboarding3 = require("../assets/images/onboarding3.png");
const Google = require("../assets/images/google.png");
const Facebook = require("../assets/images/facebook.png");
const Biometric = require("../assets/images/biometric.png");
const NotitficationIcon = require("../assets/images/notification.png");
const Setting = require("../assets/images/settings.png");
const Search = require("../assets/images/search.png");
const UserIcon = require("../assets/images/user.png");
const Favourite = require("../assets/images/favorite.png");
const Doctors = require("../assets/images/Pharmacy.png");
const Pharmacy = require("../assets/images/plea.png");
const Speciality = require("../assets/images/Specialties.png");
const Record = require("../assets/images/Record.png");
const cardioBg = require("../assets/images/cardio.png");
const dermatologyBg = require("../assets/images/dermatology.png");
const GeneralMedBg = require("../assets/images/general-med.png");
const GyneologyBg = require("../assets/images/gyneology.png");
const OdentologyBg = require("../assets/images/odotology.png");
const OncologyBg = require("../assets/images/oncology.png");
const Doctor1Bg = require("../assets/images/Dr. Daniel.png");
const Doctor2Bg = require("../assets/images/Doctor # 2.png");
const Doctor3Bg = require("../assets/images/Doctor # 3.png");
const Doctor4Bg = require("../assets/images/Doctor # 4.png");
const BookingBg = require("../assets/images/Booking.png");
const CallBg = require("../assets/images/call.png");
const VideoBg = require("../assets/images/video.png");
const QuestionBg = require("../assets/images/question.png");
const ChatBg = require("../assets/images/msg.png");
const HeartBg = require("../assets/images/heart-empt.png");
const ScheduleBg = require("../assets/images/schedule.png");
const BadgeBg = require("../assets/images/badge.png");
const ProfileSetBg = require("../assets/images/profile-s.png");
const FavourteSetBg = require("../assets/images/favorite-s.png");
const PaymentSetBg = require("../assets/images/payment-s.png");
const PrivacySetBg = require("../assets/images/privacy-s.png");
const SettingsSetBg = require("../assets/images/settings-s.png");
const HelpSetBg = require("../assets/images/help-s.png");
const LogoutSetBg = require("../assets/images/logout-s.png");
const Profile2=require("../assets/images/profile2.png")
const NotificationSettBg=require("../assets/images/notification-s.png")
const PassMangBg=require("../assets/images/passmang.png")
const HomeBg=require("../assets/images/Home.png")
const Messages=require("../assets/images/Messages.png")
const Person=require("../assets/images/User Profile.png")
const BookingHBg=require("../assets/images/Booking (1).png")
const FileAttach=require("../assets/images/fileattach.png")
const VoiceAttach=require("../assets/images/voice.png")
const SendBg=require("../assets/images/send.png")
export {
  FileAttach,
  VoiceAttach,
  SendBg,
  HomeBg,
  Messages,
  Person,
  BookingHBg,
  NotificationSettBg,
  PassMangBg,
  Profile2,
  ProfileSetBg,
  FavourteSetBg,
  PaymentSetBg,
  PrivacySetBg,
  SettingsSetBg,
  HelpSetBg,
  LogoutSetBg,
  ScheduleBg,
  BadgeBg,
  HeartBg,
  ChatBg,
  CallBg,
  VideoBg,
  QuestionBg,
  BookingBg,
  Doctor1Bg,
  Doctor2Bg,
  Doctor3Bg,
  Doctor4Bg,
  cardioBg,
  dermatologyBg,
  GeneralMedBg,
  GyneologyBg,
  OdentologyBg,
  OncologyBg,
  Favourite,
  Doctors,
  Pharmacy,
  Speciality,
  Record,
  NotitficationIcon,
  UserIcon,
  Setting,
  Search,
  Logo,
  Splashlogo,
  Onboarding1,
  Onboarding2,
  Onboarding3,
  Google,
  Facebook,
  Biometric,
};
